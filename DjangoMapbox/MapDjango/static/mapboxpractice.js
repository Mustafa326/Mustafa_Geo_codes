...
// adding the Fire data of FIRMS Global Modis 
    ...
    map.addLayer({
     ...
     map.addSource('finalshapefiletilelayersource', {
      ...
      map.addSource('ndvishapefiletilelayersource', {
        type: 'vector',
        // url: 'http://127.0.0.1:8000/finalshapefiletilelayer/{z}/{x}/{y}.pbf',
        tiles: ['http://127.0.0.1:8000/ndvishapefiletiles/{z}/{x}/{y}']
      });
      
      // Add a new layer for the vector tiles
      map.addLayer({
        id: 'pgsqlshapetwondvi',
        type: 'fill',
        source: 'ndvishapefiletilelayersource',
        'source-layer': "ndvishapefile",
        paint: {
          'fill-color': '#008000',
          'fill-opacity': 0.7
        },
...
map.on('click', 'pgsqlshapeone', function(e) {
    var districtName = e.features[0].properties.adm2_en;

    new mapboxgl.Popup()
        .setLngLat(e.lngLat)
        .setHTML('<h3>' + districtName + '</h3>')
        .addTo(map);
});

// Change the cursor to a pointer when the mouse is over the pgsqlshapeone layer.
map.on('mouseenter', 'pgsqlshapeone', function() {
    map.getCanvas().style.cursor = 'pointer';
});

// Change it back to a pointer when it leaves.
map.on('mouseleave', 'pgsqlshapeone', function() {
...
//     });
    ...
    const toggleableLayerIds = ['countries_layer', 'rgb_fog', 'rgb_dust', 'rgb_snow', 'rdri_agri','fapar_anom', 'fire_hmodis', 'fire_hviirs', 'gtd', 'ggo','nasa_eonet','pgsqlshapeone','pgsqlshapetwondvi'];
