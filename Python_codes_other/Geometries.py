...
firepit.AddPoint(53.5, 13)
vp.plot(firepit, 'rs')
print("Your new point is",firepit)

# Or edit the point using SetPoint instead of AddPoint.
firepit.SetPoint(0, 59.5, 14)
print("your point through SetPoints",firepit)
...
faucets.AddGeometry(faucet)
print("Your MUlti point Geometry is:",faucets)
...
print("Your Multi_point after editing",faucets)

# Change the coordinates back for the next example.
faucets.GetGeometryRef(1).AddPoint(73, 31)
vp.remove('tmp')
#Edit through setpoint
#faucets.SetPoint(2,114,232)
print("Your multipoint after set point is",faucets)
...
vp.plot(sidewalk, 'r--')
print("Your LineString after editing is",sidewalk)
...
print("Your vertices are",vertices)
...
print("Your new linestring is:",new_sidewalk)
...
print("The edited vertices are:",vertices)
...
print("your edited line is:",sidewalk)
'''
###########################  The Python *-operator  ###########################
'''
...
#printing the multiline and extracting geometry from it
print("Your multilinestring is:",paths)
for i in range(paths.GetGeometryCount()):
    geom=paths.GetGeometryRef(i)
    for j in range(geom.GetPointCount()):
        X=geom.GetX(i)
        Y=geom.GetY(i)
        print("The X and Y Points are",X,Y,"Respectively")
...
yard.CloseRings()
#Extracting x and y points from linerings
geom=yard.GetGeometryRef(0)
for i in range(geom.GetPointCount()):
    x=geom.GetX()
    y=geom.GetY()
    print("Your X point is:",x,"Your Y Point is:",y)
print(geom.GetPointCount())
print("Your poly gon is:",yard)
#for ds in range(yard.GetGeometryCount()):
    #geom=yard.GetGeometryRef(ds)
...
gardens.CloseRings()
print("Your MultiPolygon Geometry is:",gardens)
for d in range(gardens.GetGeometryCount()):
    geom=gardens.GetGeometryRef(d).GetGeometryRef(0)
    for f in range(geom.GetPointCount()):
        g,r=geom.GetX(),geom.GetY()
        print("Your X point is:",g,"Your Y Point is:",r)
