...
fnames = glob.glob(r'D:/Mustafa/2023_12_20_04-00/04-00_rawHrit/H*202312200400-__')
...
ir_threshold_max = 255  # maximum threshold for IR difference
vis_threshold = 5    # threshold for visible channel reflectance
...
# Examine the data to determine the appropriate threshold
# Plot histogram of the visible channels to assist with choosing a threshold
plt.figure()
plt.hist(scn['VIS006'].values.flatten(), bins=100, color='blue', alpha=0.7, label='VIS006')
plt.hist(scn['VIS008'].values.flatten(), bins=100, color='green', alpha=0.7, label='VIS008')
plt.legend()
plt.title('Histogram of Visible Channel Reflectance')
