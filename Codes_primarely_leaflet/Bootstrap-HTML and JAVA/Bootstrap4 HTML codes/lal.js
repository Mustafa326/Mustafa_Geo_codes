...
//  leaf icons
var leafLowIcon = L.icon({
  iconUrl: 'leaf_low.svg',
  iconSize: [25, 25]
});

var leafMediumIcon = L.icon({
  iconUrl: 'leaf_medium.svg',
  iconSize: [25, 25]
});

var leafHighIcon = L.icon({
  iconUrl: 'leaf_high.svg',
  iconSize: [25, 25]
});
          
// function to use different icons based on ndvi values
function ndvi2010(feature){
  var icon;
  if (feature.properties.N_2010 >= 0.4) icon = leafHighIcon;
  else if (feature.properties.N_2010 >=0.27) icon = leafMediumIcon;
  else icon = leafLowIcon;
...
function ndvi2011(feature){
  var icon;
  if (feature.properties.N_2011 >= 0.4) icon = leafHighIcon;
  else if (feature.properties.N_2011 >=0.27) icon = leafMediumIcon;
  else icon = leafLowIcon;

  return icon;
}
function ndvi2012(feature){
  var icon;
  if (feature.properties.N_2012 >= 0.4) icon = leafHighIcon;
  else if (feature.properties.N_20102 >=0.27) icon = leafMediumIcon;
  else icon = leafLowIcon;

  return icon;
}
function ndvi2013(feature){
  var icon;
  if (feature.properties.N_2013 >= 0.4) icon = leafHighIcon;
  else if (feature.properties.N_2013 >=0.27) icon = leafMediumIcon;
  else icon = leafLowIcon;

  return icon;
}
function ndvi2014(feature){
  var icon;
  if (feature.properties.N_2014 >= 0.4) icon = leafHighIcon;
  else if (feature.properties.N_2014 >=0.27) icon = leafMediumIcon;
  else icon = leafLowIcon;

  return icon;
}
function ndvi2015(feature){
  var icon;
  if (feature.properties.N_2015 >= 0.4) icon = leafHighIcon;
  else if (feature.properties.N_2015 >=0.27) icon = leafMediumIcon;
  else icon = leafLowIcon;

  return icon;
}
function ndvi2016(feature){
  var icon;
  if (feature.properties.N_2016 >= 0.4) icon = leafHighIcon;
  else if (feature.properties.N_2016 >=0.27) icon = leafMediumIcon;
  else icon = leafLowIcon;

  return icon;
}
function ndvi2017(feature){
  var icon;
  if (feature.properties.N_2017 >= 0.4) icon = leafHighIcon;
  else if (feature.properties.N_2017 >=0.27) icon = leafMediumIcon;
  else icon = leafLowIcon;

  return icon;
}
function ndvi2018(feature){
  var icon;
  if (feature.properties.N_2018 >= 0.4) icon = leafHighIcon;
  else if (feature.properties.N_2018 >=0.27) icon = leafMediumIcon;
  else icon = leafLowIcon;

  return icon;
}
function ndvi2019(feature){
  var icon;
  if (feature.properties.N_2019 >= 0.4) icon = leafHighIcon;
  else if (feature.properties.N_2019 >=0.27) icon = leafMediumIcon;
  else icon = leafLowIcon;
        ...
        //Check if feature is a polygon
            ...
            layer.on(
                'click', function(e){
                    ...
                    this.bindTooltip( 'District:'+feature.properties.Districts+'<br>'+'Precipitaion for the year 2010:'+feature.properties.P_2010+'mm per year'+'<hr>'+'The Absolute population for the year2010:'+feature.properties.AbsPop_10+'<hr>'+'The ndvi for the year2010:'+feature.properties.N_2010);
                }
            )
